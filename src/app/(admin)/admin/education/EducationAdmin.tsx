...
export default function EducationAdmin({
  ...
  const [state, formAction, pending] = useActionState<ActionState, FormData>(
    async (previous, formData) => {
      const result = await saveEducation(previous, formData);
      if (result.ok) {
        setEditing(null);
        setCreating(false);
      }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-6">
        <div>
          <h1 className="text-3xl font-medium tracking-tight">Education</h1>
      ...
      {open && (
        ...
        >
          ...
          <Field label="School" name="school" defaultValue={editing?.school} />
          <Field
            label="Field of study"
            name="fieldOfStudy"
            defaultValue={editing?.fieldOfStudy}
          />
      ...
      <EntryList
        entries={education.map((entry) => ({
          id: entry.id,
          primary: entry.degree || "(no degree)",
          secondary: entry.school,
          meta: entry.current
            ? `${entry.startDate.slice(0, 4)} – Present`
            : `${entry.startDate.slice(0, 4)} – ${entry.endDate.slice(0, 4)}`,
        }))}
        onReorder={(ids) => reorder("education", ids)}
        onDelete={deleteEducation}
        onEdit={(id) => {
          setCreating(false);
          setEditing(education.find((e) => e.id === id) ?? null);
        }}
      />
    </div>
  );
